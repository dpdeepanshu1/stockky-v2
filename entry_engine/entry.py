...
async def expire_stale_orders(db: Session, mode: str) -> int:
    ...
    # DB-side comparison against a naive `DateTime` column — bind a naive
    # UTC value here so the driver does a plain lexical/naive compare
    # instead of risking an aware-vs-naive mismatch at the SQL layer.
    now_naive = now.replace(tzinfo=None)
    stale = (
        db.query(models.TradeOrder)
        .filter(models.TradeOrder.mode == mode, models.TradeOrder.status == "PLACED",
                models.TradeOrder.valid_until.isnot(None), models.TradeOrder.valid_until < now_naive)
        .all()
    )
    for order in stale:
