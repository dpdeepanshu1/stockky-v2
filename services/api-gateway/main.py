...
# ── Hot Picks Repair All: one-shot background repair of every incomplete row ──
# ("Repair 15" only ever touched the next 15 rows per click; this walks the
# WHOLE incomplete set — price AND score/decision — in rate-safe batches, the
# same shape as the Data Feed tab's /api/feed/repair-all.)
_HOTPICKS_REPAIR_ALL_JOB: dict = {
    "status": "idle",       # idle | running | done | stopped | error
    "total": 0,
    "processed": 0,
    "ok_count": 0,
...
_HOTPICKS_REPAIR_ALL_BATCH = 10
_HOTPICKS_REPAIR_ALL_MAX_LOOPS = 300          # hard safety cap on iterations
_HOTPICKS_REPAIR_ALL_STALL_LIMIT = 4          # consecutive no-progress loops


def _run_hotpicks_repair_all_job():
    global _HOTPICKS_REPAIR_ALL_JOB
    try:
        from hotpicks_store import hotpicks_repair_batch, hotpicks_repair_scores, hotpicks_audit
    except Exception as e:
        _HOTPICKS_REPAIR_ALL_JOB.update({
            "status": "error",
            "message": f"hotpicks store unavailable: {str(e)[:160]}",
            "finished_at": datetime.now(IST).isoformat(),
        })
        return

    # Best-effort estimate for the progress bar only — the loop below keeps
    # going by attempted-count, not by this number, so an undercount (audit
    # caps incomplete_stocks at 200) never stops the job early.
    try:
        audit = hotpicks_audit()
        total_est = len(audit.get("incomplete_stocks") or [])
    except Exception:
        total_est = 0
    ...
    _HOTPICKS_REPAIR_ALL_JOB.update({
        ...
        "started_at": datetime.now(IST).isoformat(),
        "finished_at": None,
        "message": f"Repairing ~{total_est} incomplete Hot Picks rows…" if total_est else "Scanning for incomplete rows…",
        "last_symbol": None,
        "cancel_requested": False,
    })
    ...
    for _ in range(_HOTPICKS_REPAIR_ALL_MAX_LOOPS):
        if _HOTPICKS_REPAIR_ALL_JOB.get("cancel_requested"):
            _HOTPICKS_REPAIR_ALL_JOB.update({
                "status": "stopped",
                "message": f"Stopped by user after {processed} attempted, {ok_n} improved.",
                "finished_at": datetime.now(IST).isoformat(),
            })
            return

        try:
            price_res = hotpicks_repair_batch(limit=_HOTPICKS_REPAIR_ALL_BATCH, market_data_url=market_url)
        except Exception as e:
            price_res = {"status": "error", "attempted": 0, "repaired": []}
            logger.debug("repair-all price pass: %s", e)
        try:
            score_res = hotpicks_repair_scores(limit=_HOTPICKS_REPAIR_ALL_BATCH, decision_url=decision_url)
        except Exception as e:
            score_res = {"status": "error", "attempted": 0, "repaired": []}
            logger.debug("repair-all score pass: %s", e)

        attempted = (price_res.get("attempted") or 0) + (score_res.get("attempted") or 0)
        if attempted == 0:
            # Nothing left that either pass can find — genuinely done.
            break

        combined = list(dict.fromkeys(list(price_res.get("repaired") or []) + list(score_res.get("repaired") or [])))
        processed += attempted
        if combined:
            ok_n += len(combined)
            stall_streak = 0
            _HOTPICKS_REPAIR_ALL_JOB["last_symbol"] = combined[-1]
        else:
            # Rows are being selected every loop but nothing actually improves
            # (e.g. decision service can't score a delisted symbol) — without
            # this the loop would spin on the same rows forever.
            stall_streak += 1

        _HOTPICKS_REPAIR_ALL_JOB["processed"] = processed
        _HOTPICKS_REPAIR_ALL_JOB["ok_count"] = ok_n
        _HOTPICKS_REPAIR_ALL_JOB["message"] = f"{processed} attempted · {ok_n} improved so far…"

        if stall_streak >= _HOTPICKS_REPAIR_ALL_STALL_LIMIT:
            stalled_note = " (stopped — remaining rows can't be auto-repaired, e.g. no live quote or score available)"
            break
    ...
    _HOTPICKS_REPAIR_ALL_JOB.update({
        "status": "done",
        "message": (f"Done — {ok_n}/{processed} improved." if processed else "Nothing to repair — Hot Picks feed already healthy.") + stalled_note,
        "finished_at": datetime.now(IST).isoformat(),
    })


@app.post("/stockky-hot/repair-all")
def stockky_hot_repair_all(background_tasks: BackgroundTasks):
    """Kick off a background job that repairs EVERY incomplete Hot Picks row
    (price, then decision/score), not just the next 15. Poll
    /stockky-hot/repair-all/status for progress."""
    if _HOTPICKS_REPAIR_ALL_JOB.get("status") == "running":
        return {"ok": True, "already_running": True, **_HOTPICKS_REPAIR_ALL_JOB}
    background_tasks.add_task(_run_hotpicks_repair_all_job)
    return {"ok": True, "started": True, "message": "Repair All started in the background."}


@app.get("/stockky-hot/repair-all/status")
def stockky_hot_repair_all_status():
    return dict(_HOTPICKS_REPAIR_ALL_JOB)


@app.post("/stockky-hot/repair-all/stop")
def stockky_hot_repair_all_stop():
    _HOTPICKS_REPAIR_ALL_JOB["cancel_requested"] = True
    return {"ok": True, "message": "Stop requested — will halt after the current batch."}
