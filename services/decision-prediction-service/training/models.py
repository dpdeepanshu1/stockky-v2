...
def ensure_oracle_identity(engine) -> None:
    ...
    this repair every INSERT keeps failing with::

        ORA-01400: cannot insert NULL into ("ADMIN"."PREDICTION_SNAPSHOTS"."ID")

    Oracle cannot retrofit a true IDENTITY onto an existing column, so this uses
    the supported 12c+ equivalent: a sequence plus ``DEFAULT seq.NEXTVAL``.
    Because a DEFAULT only fires when the column is omitted from the INSERT
    (which is exactly what SQLAlchemy does), explicit inserts such as
    ``PortfolioAccount(id=1)`` in trades.py keep working unchanged.

    Idempotent and self-healing:
      * skips any table that does not exist yet,
      * skips any id that is already a real identity column,
      * starts the sequence at ``MAX(id) + 1`` so existing rows keep their ids
        and no PK collision is possible,
      * reuses an existing sequence (ORA-00955) on later boots,
    ...
    for table in _PK_TABLES:
        ...
        try:
            ...
            # Oracle CANNOT convert a plain column into an identity column:
            # "ALTER TABLE ... MODIFY (id GENERATED BY DEFAULT AS IDENTITY)" only
            # retunes an *existing* identity and otherwise fails with ORA-30673.
            # The supported non-destructive equivalent on 12c+ is a sequence plus
            # a column DEFAULT — SQLAlchemy omits `id` from the INSERT, so Oracle
            # applies the default and generates the value. Same end result, and
            # no table rebuild / data loss.
            seq = f"seq_{table}_id"[:30]
            try:
                with engine.begin() as conn:
                    conn.execute(
                        text(
                            f"CREATE SEQUENCE {seq} START WITH {start_at} "
                            f"INCREMENT BY 1 NOCACHE NOCYCLE"
                        )
                    )
            except Exception as e:  # noqa: BLE001
                # ORA-00955 = sequence already there from an earlier boot: reuse it.
                if "ORA-00955" not in str(e):
                    raise
            with engine.begin() as conn:
                conn.execute(text(f"ALTER TABLE {tbl} MODIFY (id DEFAULT {seq}.NEXTVAL)"))

            _log.info(
                "ensure_oracle_identity: %s.id now DEFAULT %s.NEXTVAL (start=%s)",
                tbl,
                seq,
                start_at,
            )
