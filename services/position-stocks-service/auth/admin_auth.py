...
Deliberate duplicate of real-trade-service/auth/admin_auth.py (this
service's isolation design copies rather than imports across services —
see config.py's top-of-file note). Reads the SAME ADMIN_USERNAME /
ADMIN_PASSWORD_HASH / SESSION_SECRET env vars out of the SAME .env, so the
one admin password works identically on both dashboards.

Argon2id password check against config.ADMIN_PASSWORD_HASH. Session tokens
are short-lived signed JWTs; every mutating route in main.py re-validates
via require_admin(), not just at login.

Unlike real-trade-service, this service has no DEMO/REAL mode split — it's
REAL-money-only — so there's no require_admin_if_real() here: every
mutating route uses plain require_admin().
"""
...
logger = logging.getLogger("position-stocks-auth")
...
def require_admin(authorization: str = Header(default="")) -> str:
    """FastAPI dependency — put on every route that mutates position-stocks
