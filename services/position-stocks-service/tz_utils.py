...
# day, so the background auto-pilot loop (main.py's _trading_loop) kept
# running full cycles — Dhan reconciliation calls, quality-gate calls to
# analysis-intelligence-service, etc. — on a day the exchange was actually
# closed, and notification-scheduler-service separately fired its own
# scan/notify messages for the same reason (its own stale holiday list,
# see that service's scheduler/run_once.py).
#
...
# is_market_open_ist() call site (entry_engine-equivalent gates, main.py's
# background loop, the risk-check dry-run route) gets holiday-awareness
...
#   - services/position-stocks-service/tz_utils.py    (this file)
#   - services/real-trade-service/tz_utils.py          (identical copy)
...
def is_market_open_ist(now: Optional[datetime] = None) -> bool:
    ...
    dry-run route — and gates main.py's _trading_loop background cycle."""
    ist_dt = (now or datetime.now(timezone.utc)).astimezone(IST)
