...
@app.get("/dhan/edis/request-tpin")
...
@app.get("/dhan/edis/authorize-form")
async def dhan_edis_authorize_form(
...
@app.get("/dhan/edis/status")
async def dhan_edis_status(admin: str = Depends(require_admin), db: Session = Depends(get_db), isin: str = "ALL"):
    """Read-only check: has today's CDSL authorization already been done?
    Check this before assuming an exit will fail — no need to re-run the
    T-PIN flow if aprvdQty already covers what you're about to sell."""
...
async def sell_demat_holding(
    ...
    """Manual sell for a raw Dhan demat holding — 2026-09-17 addition.

    Demat holdings pulled straight from Dhan (see /dhan/holdings above)
    are not necessarily backed by a row in trade_positions — a holding
    can exist in the demat account without this service ever having
    opened it as a tracked position (bought outside the app, or a
    partial import gap). The existing /positions/{mode}/{id}/close route
    only works for rows this service already tracks, which is exactly
    why the "Demat Holdings" table had no sell action at all before this
    fix. This route instead sells directly against the security_id/qty
    the /dhan/holdings response itself reports, with no TradePosition
    lookup required.

    REAL money only — a demat holding is a REAL-account concept; there
    is nothing to sell in DEMO. Always allowed regardless of armed
    state, same policy as closing a tracked position: exiting a real
    holding must never be blocked by "not armed"."""
    if body.quantity <= 0:
    ...
    if not lock.acquire(blocking=False):
        raise HTTPException(
            status_code=409,
            detail="An exit for REAL is already being processed — try again in a moment.",
        )
    try:
        if _has_pending_real_sell(db, body.symbol):
            raise HTTPException(
                status_code=409,
                detail=(
                    f"A SELL for {body.symbol} is already sent to Dhan and awaiting "
                    "fill confirmation — wait for it to settle before selling again."
                ),
            )
        import asyncio as _asyncio
        try:
        ...
        order = models.TradeOrder(
            mode="REAL", symbol=body.symbol, side="SELL", order_type="MARKET",
            qty=body.quantity, limit_price=None, status="PLACED",
            dhan_order_id=dhan_order_id, execution_source="MANUAL",
            confirmed_by=admin or "admin", confirmed_at=datetime.now(timezone.utc),
            exit_reason="manual",
        )
        db.add(order)
        db.commit()
        ...
        db.add(models.TradeOrderEvent(
            ...
            detail=f"Manual demat-holding sell — {body.symbol} qty={body.quantity}",
        ))
        db.commit()
        log_action(
            db, actor=admin or "admin", action="MANUAL_SELL_HOLDING", mode="REAL",
            detail=f"{body.symbol} qty={body.quantity} dhan_order_id={dhan_order_id}",
        )
        return {
            "ok": True, "mode": "REAL", "symbol": body.symbol,
            "qty": body.quantity, "dhan_order_id": dhan_order_id, "order_id": order.id,
        }
    finally:
        lock.release()
