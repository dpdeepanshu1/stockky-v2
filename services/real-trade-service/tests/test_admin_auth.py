...
Covers auth/admin_auth.py — previously 61%, missing lines 39-50, 55, 67,
71-74, 87, 129-136. This is the Layer-1 gate in front of every REAL-mode
route (real money), so it gets exhaustive branch coverage rather than just
the missing lines: every function, every branch, every failure mode.
  ...
  decode_session_token
    ...
    - happy path
  require_admin / require_admin_if_real
    - missing header, non-Bearer scheme, bad token, expired token -> 401
    - valid token -> username; DEMO bypass -> None; REAL enforced (any case)
...
class TestVerifyAdminPassword:
    ...
    def test_unexpected_verifier_error_fails_closed_and_logs_no_password(
        ...
        with caplog.at_level(logging.WARNING, logger="real-trade-auth"):
            assert verify_admin_password(USERNAME, PASSWORD) is False
...
class TestRequireAdminIfReal:
    def test_demo_needs_no_auth_and_returns_none(self):
        assert require_admin_if_real("DEMO", "") is None
        assert require_admin_if_real("demo", "Bearer garbage") is None

    def test_real_without_token_is_401(self):
        with pytest.raises(HTTPException) as ei:
            require_admin_if_real("REAL", "")
        assert ei.value.status_code == 401

    def test_real_with_valid_token_returns_username(self):
        token, _ = issue_session_token(USERNAME)
        assert require_admin_if_real("REAL", f"Bearer {token}") == USERNAME

    def test_real_mode_check_is_case_insensitive(self):
        """'real' / 'Real' must not slip through the DEMO bypass."""
        for mode in ("real", "Real", "rEaL"):
            with pytest.raises(HTTPException) as ei:
                require_admin_if_real(mode, "")
            assert ei.value.status_code == 401

    def test_unknown_mode_is_treated_as_open_like_demo(self):
        """Documents the current contract: only the literal REAL is gated;
        anything else falls to the open branch. (main.py validates `mode`
        upstream — this pins the behaviour so a change is a conscious one.)"""
        assert require_admin_if_real("PAPER", "") is None
...
class TestLogAuthConfig:
    def _run(self, caplog):
        with caplog.at_level(logging.INFO, logger="real-trade-auth"):
    ...
    def test_all_good_logs_info_only_no_errors(self, caplog):
        ...
        assert "AUTH CONFIG [real-trade-service]" in msg
    ...
    def test_missing_secret_logs_error(self, monkeypatch, caplog):
        ...
        assert "[real-trade-service]" in errors[0]
