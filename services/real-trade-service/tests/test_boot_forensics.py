...
Covers boot_forensics.py — previously 18%, missing lines 47-52, 56-71, 75,
79-85, 89-105, 109-125, 131-152, 157-180, 184-191 (i.e. everything except
...
Also pins one behaviour FIXED this session (same fix applied to the identical
copies in position-stocks-service/ and api-gateway/): record_boot used to
raise on a previous-state file that was valid JSON of the wrong shape, before
the bad file was ever replaced, so forensics stayed dead on every later boot.
...
class TestHeartbeatLoop:
    ...
    def test_at_threshold_warns_with_service_and_numbers(self, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.DEBUG, logger=LOGGER):
            _run_heartbeat(monkeypatch, max_sleeps=1, service="real-trade-service")
        warns = [r for r in caplog.records if r.levelno == logging.WARNING]
        assert len(warns) == 1
        msg = warns[0].getMessage()
        assert "MEMORY PRESSURE [real-trade-service]" in msg
...
class TestRecordBoot:
    def test_fresh_container_returns_cause_and_persists_full_state(self, monkeypatch, caplog):
        ...
        before = time.time()
        with caplog.at_level(logging.DEBUG, logger=LOGGER):
            result = bf.record_boot("real-trade-service")

        assert result["cause"] == "FRESH_CONTAINER" and "re-created" in result["detail"]
        st = _state_file()
        assert st["service"] == "real-trade-service"
        ...
        assert "BOOT FORENSICS [real-trade-service]" in msg
    ...
    def test_first_boot_installs_signal_logging_and_starts_daemon_heartbeat_once(self, spawn_stubs):
        bf.record_boot("real-trade-service")
        assert spawn_stubs == ["real-trade-service"]
        ...
        assert t.kw["args"] == ("real-trade-service",)
        assert t.kw["name"] == "boot-forensics-heartbeat"
        assert t.kw["daemon"] is True

        bf.record_boot("real-trade-service")  # re-boot in the same process
        assert spawn_stubs == ["real-trade-service"] and len(_FakeThread.instances) == 1
