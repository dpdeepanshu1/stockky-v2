...
export const realTradeApi = {
  ...
  // Manual on-demand trigger for the TOTP auto-regenerate path — for when
  // the token expired and the scheduled background refresh didn't fire in
  // time. 409s (with a clear message) if DHAN_TOTP_ENABLED isn't set on
  // this deploy, since then there's nothing to trigger.
  dhanRegenerateToken: () =>
