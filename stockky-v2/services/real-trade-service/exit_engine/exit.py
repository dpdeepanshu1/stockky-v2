...
def _send_real_sell(
    ...
    try:
        ...
        # MARKET order — actual fill price isn't known yet (reconcile.py's
        # "SELL filled" notification reports that once Dhan confirms). What
        # IS known right now: entry price, the stop/target that triggered
        # this exit, and unrealized P&L as of the last mark — enough to read
        # the alert without needing to open the dashboard.
        notify_sync(
            f"📤 *SELL sent* — {position.symbol} ×{qty} ({reason})\n"
            f"Entry ₹{position.avg_entry_price:.2f}"
            + (f" | stop ₹{position.current_stop:.2f}" if position.current_stop else "")
            + (f" | target ₹{position.current_target:.2f}" if position.current_target else "")
            + f"\nUnrealized P&L as of last mark: ₹{position.unrealized_pnl:+,.2f}\n"
