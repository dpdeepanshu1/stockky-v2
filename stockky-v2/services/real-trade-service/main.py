...
async def dhan_regenerate_token(admin: str = Depends(require_admin), db: Session = Depends(get_db)):
    """Manual, on-demand trigger for the same TOTP-based auto-regenerate path
    execution/auto_pilot.py's background _totp_refresh_loop already calls on a
    schedule (dhan_credentials.refresh_if_totp_enabled). For when the token
    expired and the scheduled refresh didn't fire in time — loop hadn't ticked
    yet, service was mid-restart, margin math left less runway than expected —
    and you don't want to wait for the next tick or paste a token by hand.

    Same requirements as the background loop: DHAN_TOTP_ENABLED=true and
    DHAN_TOTP_SECRET / DHAN_CLIENT_ID / DHAN_PIN set in env. If TOTP isn't
    configured on this deploy (the default), this 409s with guidance to use
    the manual paste flow ("Rotate token") instead — it does not silently
    no-op, so the button always tells you which path actually ran.
    """
    if not config.DHAN_TOTP_ENABLED:
        raise HTTPException(
            status_code=409,
            detail=(
                "DHAN_TOTP_ENABLED=false on this deploy — automatic token "
                "regeneration isn't configured. Use \"Rotate token\" to paste a "
                "fresh token manually, or set DHAN_TOTP_ENABLED=true plus "
                "DHAN_TOTP_SECRET/DHAN_CLIENT_ID/DHAN_PIN to enable this button."
            ),
        )
    ok = dhan_credentials.refresh_if_totp_enabled(db)
    log_action(
        db, actor=admin, action="DHAN_TOKEN_MANUAL_REGENERATE",
        detail="ok" if ok else "failed", mode="REAL",
    )
    if not ok:
        raise HTTPException(
            status_code=502,
            detail=(
                "Dhan token regeneration failed — check server logs for the "
                "TOTP/Dhan API error (bad secret, wrong PIN, Dhan endpoint "
                "unreachable, etc.). Paste a fresh token manually as a fallback."
            ),
        )
    return dhan_credentials.connection_status(db)
